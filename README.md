...
大型媒体库中出现少量异常图片是正常情况，单张图片不能阻塞 1000+ 张任务。v1.1.2 将错误分成两类：
...
因此，类似 `Failed to download image` 的错误不会再让整个全站任务停在同一张图片上。失败图片仍保留在“失败”列表，可以在主任务结束后统一重试或人工处理。

当任务确实因全局错误暂停时，后台会显示当前 Attachment，并提供：

- **重新尝试 / 继续**：修复 API 或配置问题后重试当前图片；
- **跳过当前图片并继续**：人工确认后跳过当前阻塞项，继续后面的队列；
- **停止任务**：保留已经完成的结果，不回滚。
...
- 单张图片下载失败、格式异常或图片输入错误会记录为失败并自动继续下一张；
- API Key / 额度 / 权限 / Model 等全局错误才暂停整个任务；
- 全局错误暂停时可“重新尝试 / 继续”，也提供“跳过当前图片并继续”的人工兜底。
...
插件优先读取 WordPress 本地文件，并以 Base64 方式发送给 Vision API。v1.1.2 起进一步避免把大尺寸原图直接交给远程下载器。
...
- 中文、空格及其他非 ASCII 图片文件名不再依赖远程 URL 下载；
- 发送约 1024px 的 WordPress 中间尺寸，降低批量任务的数据量和失败概率。
...
8 MiB 以内的本地原图
↓
最后才使用 Attachment URL fallback
```

可读取的本地图片会编码为 Base64。只有本地文件和可用中间尺寸都无法使用时，才回退到公开 URL；URL 路径中的中文或其他非 ASCII 字符会进行百分号编码。

> URL fallback 仍要求 DeepSeek 能访问该图片，因此它只是最后的兼容方案，不是正常批量处理的首选路径。
